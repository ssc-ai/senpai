...
from senpai.engine.detection.point.satellite_extra import (
    extract_point_sources as extract_satellite_points,
)
from senpai.engine.detection.point.sidereal_extra import (
...
def _processed_image(data: np.ndarray, image_id: str = "synthetic") -> ProcessedFitsImage:
    """Wrap raw pixel data in a :class:`ProcessedFitsImage` with basic metadata.

    Args:
        data: The 2D image pixel data.
        image_id: Identifier stored on the image metadata.
...
def test_detect_sources_classic_finds_known_stars() -> None:
    """The classic finder recovers every planted star within tolerance."""
...
def test_detect_sources_classic_positions_within_one_pixel() -> None:
    """Recovered centroids land within one pixel of the truth positions."""
...
def test_detect_sources_classic_respects_max_sources() -> None:
    """The classic finder returns no more than ``max_sources`` detections."""
...
def test_detect_sources_classic_empty_on_blank_field() -> None:
    """A blank noise field yields no classic-finder detections."""
...
def test_estimate_fwhm_recovers_known_sigma(sigma: float) -> None:
    """The FWHM estimate recovers the known Gaussian sigma."""
    x, y = 64.0, 64.0
...
def test_estimate_fwhm_returns_none_for_tiny_box() -> None:
    """An edge box too small to fit returns ``None``."""
...
def test_extract_point_sources_counts_and_positions() -> None:
    """Point-source extraction recovers all planted stars at their positions."""
...
def test_extract_point_sources_reports_reasonable_fwhm() -> None:
    """The reported FWHM matches the synthetic PSF width."""
    sigma = 2.5
...
def test_extract_point_sources_faint_below_threshold_not_detected() -> None:
    """A bright star is detected while a noise-floor faint star is not."""
    bright = (64.0, 64.0)
    faint = (180.0, 180.0)
...
def test_extract_point_sources_respects_max_detections() -> None:
    """Extraction returns no more than ``max_detections`` detections."""
...
def test_extract_point_sources_min_separation_dedupes() -> None:
    """Two stars closer than the minimum separation collapse to one detection."""
...
def test_extract_point_sources_empty_field_returns_default_fwhm() -> None:
    """An empty field returns no detections and the default FWHM."""
...
def test_measure_fwhm_from_catalog_stars_recovers_sigma() -> None:
    """Catalog-star FWHM measurement recovers the synthetic PSF width."""
    sigma = 2.0
...
def test_measure_fwhm_skips_stars_without_positions() -> None:
    """Catalog stars lacking pixel positions are skipped in the measurement."""
    sigma = 2.0
...
def test_measure_fwhm_empty_catalog_falls_back_to_initial() -> None:
    """An empty catalog falls back to the initial FWHM value."""
...
def _rate_frame(data: np.ndarray, pixel_fwhm: float = 4.0) -> RateTrackFrame:
    """Wrap frame data in a rate-track frame with an empty star field.

    Args:
        data: The frame pixel data.
        pixel_fwhm: Seed pixel FWHM stored on the detection metadata.
...
def test_satellite_extract_detects_point_source() -> None:
    """The rate-frame detector finds the bright target above the SNR threshold."""
    cfg = get_config()
...
def test_satellite_extract_high_snr_threshold_filters_all() -> None:
    """An impossibly high SNR threshold filters out every detection."""
    sigma = 2.0
...
def test_satellite_extract_assigns_no_radec_without_wcs() -> None:
    """Without a WCS the detector leaves RA/Dec unset but reports a pixel FWHM."""
    sigma = 2.0
...
def _centroid_rms(
    ...
    Args:
        detected: Detected ``(x, y)`` centroid positions.
        truth: Ground-truth ``(x, y)`` star positions.
        tol: Maximum match distance in pixels.

    Returns:
        A tuple ``(n_matched, rms)``; ``rms`` is ``inf`` when nothing matched.
    """
    residuals = []
    for tx, ty in truth:
        best = None
...
def _track_refiner_calls(monkeypatch: pytest.MonkeyPatch) -> list[int]:
    ...
    Args:
        monkeypatch: Fixture used to patch the refiner with a recording wrapper.

    Returns:
        A list that grows by one entry per refiner invocation.
    """
    import senpai.engine.detection.point.sidereal_extra as sid

    calls: list[int] = []
    original = sid._refine_centroid_full_res

    def wrapper(*args: object, **kwargs: object) -> tuple[float, float]:
        """Record the call and delegate to the original refiner."""
        calls.append(1)
...
def test_binned_pass2_runs_and_keeps_subpixel_accuracy(monkeypatch: pytest.MonkeyPatch) -> None:
    """A fat-PSF large frame takes the binned path yet keeps sub-pixel accuracy."""
...
def test_small_fwhm_large_frame_stays_unbinned(monkeypatch: pytest.MonkeyPatch) -> None:
    """A well-sampled PSF on a large frame stays on the full-res path."""
...
def test_fwhm_pass_measures_full_frame_even_with_sparse_center() -> None:
    """The FWHM pass scans the full frame, so an empty center does not bias it."""
...
def test_satellite_threshold_search_matches_daostarfinder() -> None:
    ...
    This pins the private-API reimplementation
    (``_StarFinderKernel`` / ``_DAOStarFinderCatalog``): if a photutils upgrade
    moves those internals, this fails loudly.
    """
...
def test_catalog_fwhm_skips_clipped_stars_and_measures_truth() -> None:
    """Saturated (clipped) stars are skipped; the FWHM comes from clean stars."""
...
def test_catalog_fwhm_uses_detection_sat_level_when_provided() -> None:
    """An explicit saturation level is honored rather than re-estimated."""
    sigma = 3.0
...
def test_catalog_fwhm_reads_profile_width_on_winged_psf() -> None:
    """On a winged PSF the measured FWHM is the composite half-max width."""
