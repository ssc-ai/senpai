...
def _write_fits(path: Path, header_items: dict[str, object] | None = None) -> Path:
    """Write a tiny 4x4 zero-valued FITS file with optional header cards.

    Args:
        path: Destination path for the FITS file.
        header_items: Optional mapping of header keyword to value to set.

    Returns:
        The path the FITS file was written to.
    """
...
def test_uct_time_from_combined_date_obs() -> None:
    """A combined DATE-OBS timestamp parses to the expected datetime."""
...
def test_uct_time_from_separate_date_and_time() -> None:
    """Separate DATE_OBS and TIME-OBS headers combine into one timestamp."""
...
def test_uct_time_date_only_without_time_raises() -> None:
    """A date-only header with no time component raises AttributeError."""
...
def test_uct_time_no_date_headers_raises() -> None:
    """A header with no date/time keys raises AttributeError."""
    with pytest.raises(AttributeError):
        fo.extract_uct_time_from_header({"FOO": "bar"})


def test_uct_time_prefers_date_time_over_separate() -> None:
    """A combined DATE-OBS timestamp wins over separate date/time fields."""
...
def test_parse_date_string_iso() -> None:
    """An ISO ``YYYY-MM-DD`` date string parses correctly."""
    a = fo._parse_date_string("2021-07-04")
    assert (a.year, a.month, a.day) == (2021, 7, 4)


def test_parse_date_string_us_slash_format() -> None:
    """A US ``M/D/YY`` slash-delimited date string parses correctly."""
    a = fo._parse_date_string("3/14/21")
    assert (a.month, a.day, a.year) == (3, 14, 2021)


def test_parse_time_string_with_microseconds() -> None:
    """A time string with fractional seconds parses microseconds out."""
    assert fo._parse_time_string("21:36:28.604000") == (21, 36, 28, 604000)


def test_parse_time_string_without_microseconds() -> None:
    """A time string without a fractional part yields zero microseconds."""
    assert fo._parse_time_string("08:05:09") == (8, 5, 9, 0)


def test_parse_time_string_invalid_returns_none() -> None:
    """An unparseable time string returns None."""
...
def test_get_imageset_by_filename_matches_substring(tmp_path: Path) -> None:
    """Frames whose filename contains the substring are selected, in order."""
...
def test_get_imageset_by_filename_recurses_subdirs(tmp_path: Path) -> None:
    """Filename matching recurses into subdirectories."""
    sub = tmp_path / "night1"
    sub.mkdir()
...
def test_get_imageset_by_filename_no_match(tmp_path: Path) -> None:
    """A substring matching no filename returns an empty list."""
    _write_fits(tmp_path / "alpha.fits")
    assert fo.get_imageset_by_filename(tmp_path, "zzz") == []


def test_get_all_images_in_directory_sorted(tmp_path: Path) -> None:
    """All FITS files in a directory are returned sorted by name."""
...
def test_get_all_images_in_directory_empty(tmp_path: Path) -> None:
    """An empty directory yields an empty image list."""
...
def test_extract_id_from_header_plain_key(tmp_path: Path) -> None:
    """A plain header keyword returns its value verbatim."""
    f = _write_fits(tmp_path / "img.fits", {"IMAGESET": "abc123"})
    assert fo.extract_id_from_header(f, "IMAGESET") == "abc123"


def test_extract_id_from_header_missing_key_returns_none(tmp_path: Path) -> None:
    """A missing header keyword returns None."""
    f = _write_fits(tmp_path / "img.fits", {"OTHER": "x"})
    assert fo.extract_id_from_header(f, "IMAGESET") is None


def test_extract_id_from_header_orchcomm_parsing(tmp_path: Path) -> None:
    """The special ORCHCOMM encoding is parsed down to its set name."""
    f = _write_fits(tmp_path / "img.fits", {"ORCHCOMM": "&MYSET01@[ukr]#[1:6]%[OPEN]"})
    assert fo.extract_id_from_header(f, "ORCHCOMM") == "MYSET01"


def test_header_key_matches_true_and_false(tmp_path: Path) -> None:
    """A header key matches its exact value and rejects a different one."""
...
def test_header_key_matches_missing_key_false(tmp_path: Path) -> None:
    """Matching against a missing header key is False."""
    f = _write_fits(tmp_path / "img.fits", {"OTHER": "v"})
    assert fo.header_key_matches(f, "IMAGESET", "abc") is False


def test_header_key_matches_orchcomm_substring(tmp_path: Path) -> None:
    """ORCHCOMM matching keys on the decoded set name, not the raw string."""
...
def test_get_imageset_by_id_groups_matching_headers(tmp_path: Path) -> None:
    """Frames are grouped by a shared header-ID keyword value."""
...
def test_get_imageset_by_id_no_match_empty(tmp_path: Path) -> None:
    """A header-ID value matching no frame returns an empty list."""
    _write_fits(tmp_path / "f1.fits", {"IMAGESET": "night-A"})
