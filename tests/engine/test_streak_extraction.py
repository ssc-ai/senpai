...
from senpai.engine.detection.streak.extraction_extra import refine_robust_streak
...
def test_measures_clean_streak_length(length: int) -> None:
    """A clean streak's measured length and FWHM recover the truth."""
...
def test_fragmented_streak_not_truncated() -> None:
    """A half-level gap mid-trail does not shorten the measured length."""
...
def test_hot_pixels_do_not_inflate_or_break() -> None:
    """Off-trail hot pixels neither inflate the length nor break the fit."""
...
def test_fragmented_plus_hot() -> None:
    """A fragmented trail with hot pixels still measures its true length."""
...
def test_rotated_streak() -> None:
    """A rotated streak's length is recovered along its own axis."""
...
class TestSeedEstimate:
    """Tests for the streak seed estimator on synthetic streak fields."""

    def test_recovers_ballpark_seed_not_frame_fraction(self) -> None:
        """The seed tracks the actual streak, not a fixed frame fraction."""
        from senpai.engine.detection.streak.extraction_extra import _estimate_streak_seed
...
class TestStreakCenterExtraction:
    ...
    ) -> tuple[np.ndarray, list[tuple[float, float]], StreakMetadata]:
        """Build a grid of identical streaks with known centers.

        Args:
            length: Streak length in pixels.
            rot: Streak orientation in degrees.
            size: Side length of the square image in pixels.
            seed: Seed for the deterministic RNG.

        Returns:
            A tuple ``(image, centers, streak_metadata)`` where ``centers`` are
            the planted ``(x, y)`` streak-center positions.
        """
        from senpai.engine.models.metadata import StreakMetadata
    ...
    def test_one_centroid_per_streak_at_known_centers(self) -> None:
        """Each planted streak yields exactly one centroid at its center."""
    ...
    def test_noise_field_respects_caps_and_separation(self) -> None:
        """On pure noise the count cap and minimum separation still hold."""
