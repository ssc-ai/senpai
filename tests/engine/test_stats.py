...
def _field(
    shape: tuple[int, int] = (2400, 2400),
    background: float = 120.0,
    noise: float = 8.0,
    seed: int = 11,
) -> np.ndarray:
    """Build a synthetic background field seeded with bright contaminants.

    Args:
        shape: Output image shape ``(height, width)`` in pixels.
        background: Constant background level added to every pixel.
        noise: Standard deviation of the Gaussian read noise.
        seed: Seed for the random number generator (reproducibility).

    Returns:
        A 2D float image with Gaussian background noise plus injected bright
        sources (stars / hot pixels) the sigma clip must reject.
    """
...
def test_strided_stats_match_full_computation() -> None:
    """Strided stats match full ``sigma_clipped_stats`` at the noise scale."""
    image = _field()
...
def test_small_images_are_not_subsampled() -> None:
    """Images at or below the target size are used in full (identical results)."""
...
def test_gradient_background_is_sampled_fairly() -> None:
    """A vignetting gradient is sampled frame-wide, tracking the global median."""
