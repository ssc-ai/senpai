"""Regression tests for streak masking: vectorized removers and bounded flood fill.
...
def test_remove_near_saturation_removes_only_saturated_blobs() -> None:
    """Only near-saturation blobs are filled; fainter sources are untouched."""
...
def test_remove_near_saturation_noop_when_unsaturated() -> None:
    """A frame with no near-saturation pixels is returned unchanged."""
...
def test_remove_border_crossing_removes_only_border_seeded_blobs() -> None:
    """Border-touching blobs are removed while interior blobs survive."""
    img = np.full((60, 60), 100.0)
...
def test_flood_fill_is_bounded_on_fully_connected_frame() -> None:
    """The flood fill stays capped on a fully-connected above-threshold frame."""
