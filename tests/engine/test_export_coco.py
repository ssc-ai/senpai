"""Tests for COCO export and dataset splitting.

Cover ``senpai.export.coco`` and ``senpai.export.dataset_split``. Synthetic
``SenpaiRunResult`` objects are built with serializable sidereal and
...
if TYPE_CHECKING:
    ...
    from senpai.engine.models.metadata import ImageMetadata, StreakMetadata
...
def _write_fits(path: Path, width: int = 64, height: int = 48) -> tuple[str, int, int]:
    ...
    Returns:
        The string path plus the width and height that were written.
    """
...
def _image_metadata(width: int, height: int) -> ImageMetadata:
    """Build ImageMetadata sized to the given dimensions.
    ...
    Returns:
        An ImageMetadata instance with a fixed image id.
    """
...
def _starfield(
    ...
    *,
    ...
    catalog: list[dict[str, float]] | None = None,
) -> StarField:
    """Build a StarField from raw detection/catalog dicts.

    Args:
        width: Image width in pixels.
        height: Image height in pixels.
        fit: Whether the StarField reports a successful astrometric fit.
        detections: Detection kwargs passed to each StarInImage.
        catalog: Catalog-star kwargs passed to each StarInSpace, or None.

    Returns:
        A StarField with a null WCS.
    """
...
def _sidereal_frame(
    ...
    *,
    ...
    """Build a serializable sidereal frame pointing at a FITS file.

    Args:
        fits_path: Path to the frame's processed FITS file.
        width: Image width in pixels.
        height: Image height in pixels.
        index: Frame index within the run.
        detections: Star detection kwargs for the frame's StarField.
        catalog: Catalog-star kwargs for the frame's StarField.
        fit: Whether the frame's StarField reports a successful fit.

    Returns:
        A SiderealFrameSerializable instance.
    """
...
def _streak(length: float = 20.0, angle_deg: float = 0.0, fwhm: float = 3.0) -> StreakMetadata:
    """Build StreakMetadata from a length and angle.

    Args:
        length: Streak length in pixels.
        angle_deg: Streak orientation in degrees.
        fwhm: Full width at half maximum of the streak.

    Returns:
        A StreakMetadata instance with sine/cosine derived from the angle.
    """
...
def _satellite(x: float, y: float, snr: float = 12.0) -> SatelliteInImage:
    """Build a SatelliteInImage at the given pixel position.

    Args:
        x: Satellite pixel x-coordinate.
        y: Satellite pixel y-coordinate.
        snr: Signal-to-noise ratio of the detection.

    Returns:
        A SatelliteInImage instance.
    """
...
def _rate_frame(
    ...
    *,
    ...
    """Build a serializable rate-track frame pointing at a FITS file.
    ...
    Args:
        ...
        height: Image height in pixels.
        index: Frame index within the run.
        satellites: Satellite detections for the frame, or None.
        detections: Star detection kwargs for the frame's StarField.
        streak: Streak metadata for the frame, or None.
        fit: Whether the frame's StarField reports a successful fit.

    Returns:
        A RateTrackFrameSerializable instance.
    """
...
def _run(
    ...
    """Build a SenpaiRunResult wrapping the given frames.

    Args:
        sidereal: Sidereal frames for the run, or None.
        rate: Rate-track frames for the run, or None.
        scale_factor: Optional coordinate scale factor applied on export.

    Returns:
        A completed SenpaiRunResult.
    """
...
def test_sidereal_point_annotations_bbox(tmp_path: Path) -> None:
    """Sidereal point sources export as bboxes centered on each detection."""
...
def test_sidereal_skipped_when_no_wcs(tmp_path: Path) -> None:
    """An unsolved sidereal frame produces no COCO output."""
    fits_path, w, h = _write_fits(tmp_path / "side.fits")
...
def test_process_sidereal_flag_disables_sidereal(tmp_path: Path) -> None:
    """process_sidereal=False suppresses all sidereal export output."""
    fits_path, w, h = _write_fits(tmp_path / "side.fits")
    dets = [{"x": 5.0, "y": 5.0, "counts": 10_000.0}]
    run = _run(sidereal=[_sidereal_frame(fits_path, w, h, detections=dets)])
...
def test_snr_cut_filters_low_snr_detections(tmp_path: Path) -> None:
    """The snr_cut threshold drops detections below the configured SNR."""
    fits_path, w, h = _write_fits(tmp_path / "side.fits")
...
def test_rate_satellite_and_streak_annotations(tmp_path: Path) -> None:
    """Rate frames export satellite point bboxes and star streak lines."""
...
def test_rate_streak_line_angle(tmp_path: Path) -> None:
    """A streak's angle sets the exported line start point and direction."""
    fits_path, w, h = _write_fits(tmp_path / "rate.fits")
...
def test_rate_skipped_when_streak_exceeds_max_length(tmp_path: Path) -> None:
    """A streak longer than max_streak_length is skipped on export."""
    fits_path, w, h = _write_fits(tmp_path / "rate.fits")
    star_dets = [{"x": 30.0, "y": 24.0, "counts": 500_000.0}]
...
def test_rate_skipped_when_no_wcs(tmp_path: Path) -> None:
    """An unsolved rate frame produces no COCO output."""
    fits_path, w, h = _write_fits(tmp_path / "rate.fits")
...
def test_scale_factor_scales_coordinates(tmp_path: Path) -> None:
    """A run scale_factor multiplies exported centroid coordinates."""
...
def test_write_fits_creates_image_file(tmp_path: Path) -> None:
    """write_fits=True writes a per-frame FITS image referenced in the COCO json."""
    fits_path, w, h = _write_fits(tmp_path / "side.fits")
    dets = [{"x": 10.0, "y": 10.0, "counts": 1_000_000.0}]
    run = _run(sidereal=[_sidereal_frame(fits_path, w, h, detections=dets)])
...
def test_export_batch_processes_multiple_runs(tmp_path: Path) -> None:
    """export_batch writes COCO output for each run/collect pair."""
...
def test_dataset_split_ratios_must_sum_to_one() -> None:
    """DatasetSplit rejects ratios that do not sum to one."""
    with pytest.raises(ValueError):
        DatasetSplit(train=0.5, val=0.2, test=0.1)


def test_dataset_split_defaults_valid() -> None:
    """The default DatasetSplit ratios are 0.7/0.2/0.1."""
    s = DatasetSplit()
...
def _make_point_annotation_file(
    ...
    """Write a COCO point-annotation JSON file for one image.

    Args:
        input_dir: Directory the annotation file is written into.
        image_id: COCO image id.
        file_name: Image file name recorded in the annotation.
        frame_type: Frame type tag (``"rate"`` or ``"sidereal"``).
        n_ann: Number of annotations to emit for the image.
    """
    data = {
        "images": [
            {
...
def _touch_image(input_dir: Path, file_name: str) -> None:
    """Write a tiny placeholder FITS image into the input directory.

    Args:
        input_dir: Directory the image is written into.
        file_name: Image file name to create.
    """
...
def test_dataset_split_raises_without_annotations(tmp_path: Path) -> None:
    """Splitting an annotation-free directory raises ValueError."""
...
def test_dataset_split_deterministic_with_seed(tmp_path: Path) -> None:
    """The same random seed yields the same train/val/test partition."""
    input_dir = tmp_path / "in"
    input_dir.mkdir()
    ...
    def run(seed: int) -> dict[str, list[str]]:
        """Run the split for a given seed.

        Args:
            seed: Random seed for the split.

        Returns:
            The split-name to image-id-list mapping.
        """
...
def test_dataset_split_sizes_match_ratios(tmp_path: Path) -> None:
    """Split sizes match the requested train/val/test ratios."""
    input_dir = tmp_path / "in"
    input_dir.mkdir()
    for i in range(10):
...
def test_dataset_split_writes_combined_annotation_files(tmp_path: Path) -> None:
    """The split writes combined points_/lines_ annotation files per split."""
    input_dir = tmp_path / "in"
    input_dir.mkdir()
    for i in range(4):
        fname = f"frame_{i}.fits"
