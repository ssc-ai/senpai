"""Synthetic-image tests for the live photometry engine (photometry.utils).
...
def _cfg(**kw: float) -> SimplePhotometryConfig:
    ...
    Args:
        **kw: Overrides forwarded to :class:`SimplePhotometryConfig`.

    Returns:
        The configured :class:`SimplePhotometryConfig`.
    """
...
def _inject_star(
    data: np.ndarray, x0: float, y0: float, total_flux: float, sigma: float = SIGMA
) -> None:
    """Add a normalised 2D Gaussian of given total flux in-place.

    Args:
        data: Image array modified in place.
        x0: Star centre x-coordinate in pixels.
        y0: Star centre y-coordinate in pixels.
        total_flux: Total flux of the injected Gaussian.
...
def _image(data: np.ndarray, exposure_time: float = 1.0) -> ProcessedFitsImage:
    """Wrap a data array in a :class:`ProcessedFitsImage`.

    Args:
        data: Image data array.
        exposure_time: Exposure time in seconds recorded in the metadata.

    Returns:
        The wrapped :class:`ProcessedFitsImage`.
    """
    h, w = data.shape
...
def _flux_for_mag(mag: float) -> float:
    """Return the flux corresponding to ``mag`` at the true zero-point.

    Args:
        mag: Catalog magnitude.

    Returns:
        The flux in ADU consistent with ``TRUE_ZP`` at 1s exposure.
    """
...
def _single_star_image(
    ...
    """Build an image with one injected Gaussian star on a noisy background.

    Args:
        total_flux: Total flux of the injected star.
        x0: Star centre x-coordinate in pixels.
        y0: Star centre y-coordinate in pixels.
        size: Side length of the square frame in pixels.
        bg: Constant background level in ADU.
        sky: Sky-noise standard deviation in ADU.
        seed: Seed for the random-number generator.

    Returns:
        The image and the star's ``(x0, y0)`` centre.
    """
...
def _fwhm_stats(fwhm: float = FWHM) -> FWHMMetadata:
    """Build FWHM metadata centred on a single FWHM value.

    Args:
        fwhm: Median/mean FWHM in pixels.

    Returns:
        The populated :class:`FWHMMetadata`.
    """
    return FWHMMetadata(
        n_measurements=10,
        median_fwhm=fwhm,
...
def _starfield(stars: list[StarInSpace], fwhm: float = FWHM, size: int = 400) -> StarField:
    """Build a starfield from catalog stars with a fixed FWHM.

    Args:
        stars: Catalog stars to place in the field.
        fwhm: FWHM in pixels for the field's FWHM statistics.
        size: Side length of the square frame in pixels.

    Returns:
        The constructed :class:`StarField`.
    """
...
def test_brighter_star_has_higher_snr() -> None:
    """A brighter injected star yields a higher measured SNR at equal noise."""
...
def test_single_star_background_recovered() -> None:
    """Aperture photometry recovers the injected background level and noise."""
...
def test_instrumental_magnitude_matches_flux() -> None:
    """The instrumental magnitude equals -2.5*log10(flux)."""
...
def test_star_none_coordinates_returns_none() -> None:
    """A star with no pixel coordinates cannot be measured, returning None."""
...
def test_star_near_edge_returns_none() -> None:
    """A star whose aperture leaves the frame edge returns None."""
...
def test_quality_flag_true_for_bright_isolated_star() -> None:
    """A bright, isolated star passes the quality flag."""
...
def _field_image_and_starfield(
    ...
    """Build a multi-star field image and its matching starfield.
    ...
    Returns:
        The image and the :class:`StarField` describing its stars.
    """
    rng = np.random.default_rng(seed)
...
def test_starfield_recovers_all_star_fluxes() -> None:
    """Field photometry recovers each injected star's flux to a few percent."""
...
def test_starfield_zero_point_recovered() -> None:
    """The field summary recovers the true zero-point."""
    img, sf = _field_image_and_starfield()
...
def test_starfield_summary_counts_and_snr() -> None:
    """The summary reports consistent star counts, SNR, and paired mag/SNR arrays."""
    img, sf = _field_image_and_starfield()
...
def test_starfield_records_circular_aperture_geometry() -> None:
    """Record the literal circular aperture/annulus pixel dims in the summary.
...
def test_run_result_records_aperture_policy() -> None:
    """Emit a run-level photometry block carrying the aperture PSF-factor policy.
...
def test_starfield_no_fwhm_returns_empty() -> None:
    """A starfield lacking FWHM statistics yields no photometry results."""
    img, sf = _field_image_and_starfield()
...
def test_starfield_no_catalog_stars_returns_empty() -> None:
    """A starfield with no catalog stars yields no photometry results."""
    img, sf = _field_image_and_starfield()
...
def test_assess_quality_rejects_negative_flux() -> None:
    """Negative flux always fails the quality assessment."""
    assert _assess_simple_quality(-1.0, 50.0, 0.0, _cfg()) is False


def test_assess_quality_rejects_low_snr() -> None:
    """SNR below the configured minimum fails the quality assessment."""
...
def test_assess_quality_rejects_high_crowding() -> None:
    """Crowding above the configured maximum fails the quality assessment."""
...
def test_crowding_zero_for_isolated_star() -> None:
    """An isolated star has near-zero crowding."""
...
def test_crowding_higher_with_bright_neighbor() -> None:
    """A bright neighbour just outside the aperture raises the crowding factor."""
...
def test_has_bright_neighbor_detects_blend() -> None:
    """A nearby brighter star is detected as a blend."""
...
def test_has_bright_neighbor_ignores_distant_or_faint() -> None:
    """Distant or comparably faint neighbours are not counted as blends."""
    faint = StarInSpace(ra=0, dec=0, x=100.0, y=100.0, magnitude=18.0)
...
def test_get_best_magnitude_prefers_order() -> None:
    """The preferred filter (Johnson_V) outranks a lower-priority one."""
...
def test_get_best_magnitude_falls_back_to_primary() -> None:
    """With no filter magnitudes, the primary magnitude is used."""
...
def test_get_best_magnitude_none_when_absent() -> None:
    """A star with no magnitude information returns None."""
...
def test_find_common_magnitude_system_picks_covered_filter() -> None:
    """A filter carried by all stars is chosen as the common system."""
...
def test_find_common_magnitude_system_primary_fallback() -> None:
    """With only primary magnitudes, the common system falls back to 'primary'."""
...
def test_find_common_magnitude_system_empty() -> None:
    """An empty star list has no common magnitude system."""
    assert _find_common_magnitude_system([]) is None


def test_precompute_star_magnitudes_uses_common_system() -> None:
    """Precomputed magnitudes come from the common filter system."""
...
def test_isotonic_completeness_is_monotone_decreasing() -> None:
    """Isotonic regression makes a spiky completeness curve monotone decreasing."""
...
def test_completeness_limits_crossings() -> None:
    """The 50%/90% limiting magnitudes are ordered and land in the crossing region."""
...
def test_completeness_limits_none_for_short_curve() -> None:
    """A two-point curve is too short to derive limiting magnitudes."""
    assert _completeness_limits([10, 11], [100, 50]) == (None, None, None)


def test_completeness_limits_none_when_never_crosses() -> None:
    """A curve that never drops below the target yields no limiting magnitudes."""
    mag = [10, 11, 12, 13, 14]
...
def test_compute_completeness_curve_rolls_over() -> None:
    """Completeness curve rolls from ~100% (bright) toward 0 (faint).
...
def test_compute_completeness_curve_empty_when_too_few() -> None:
    """Too few results produce empty completeness arrays."""
