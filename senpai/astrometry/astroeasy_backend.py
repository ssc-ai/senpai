...
from senpai.engine.models.starfield import StarField, StarInImage, StarInSpace, StarListImage
...
def _solve_field_cascade(sources: StarListImage, wcs: WCSModel | None, config: AppConfig) -> StarField:
    ...
    Args:
        sources: Detected sources with pixel coordinates and image metadata.
        wcs: Optional existing WCS to verify/refine.
        config: Application configuration providing the astrometry and catalog
            settings for the cascade.

    Returns:
        A StarField with the cascade's WCS solution, matched stars, and fit
        status.
    """
...
def _extract_stars_image2xy(
    ...
    Args:
        ...
        max_sources (int | None): cap; None uses ``astrometry.max_sources``.

    Returns:
        list[StarInImage]: list of StarInImage objects, capped at max_sources.
    """
    config = get_or_initialize_config()
    cap = config.astrometry.max_sources if max_sources is None else max_sources
...
def _extract_stars(fits_img: ProcessedFitsImage) -> list[StarInImage]:
    ...
    ``astrometry.source_extractor``.

    Args:
        fits_img (ProcessedFitsImage): preprocessed FITS image to extract sources from.

    Returns:
        list[StarInImage]: detected sources, capped at ``astrometry.max_sources``.
    """
    config = get_or_initialize_config()
    extractor = config.astrometry.source_extractor
    if extractor == "image2xy":
        return _extract_stars_image2xy(fits_img)
    sources, _, _ = extract_point_sources(
        fits_img,
        max_detections=config.astrometry.max_sources,
        method=extractor,
    )
    return sources.detections


def solve_field_fits(fits_img: ProcessedFitsImage) -> StarField:
    """Plate-solve a preprocessed FITS image (the detection-pipeline entry).

    Extracts point sources in-process using the extractor set by
    ``astrometry.source_extractor``, then solves them through :func:`solve_field`
    (astroeasy sources mode: real ``solve-field`` with scale bounds, boresight
    position hints from the frame metadata, and a ``--verify --tag-all`` pass).

    Args:
        fits_img (ProcessedFitsImage): preprocessed FITS image to solve.

    Returns:
        StarField: solved StarField with ``fit=True`` and a valid WCS, or an
            unfit StarField (``fit=False``, ``wcs=None``) when extraction found
            nothing or no astrometric solution was accepted.
    """
    config = get_or_initialize_config()
    if config.astrometry.solver_mode == "senpai":
        # In-process solver: blind solve + same-cell verify + SIP-1 refine,
        # replicating `solve-field --verify --tweak-order 1` (see runner module).
        from senpai.astrometry.runner import solve_field as solve_field_senpai

        return solve_field_senpai(fits_img)
    ...
    logger.info("Extracted %d stars for plate solving", len(detections))
    sources = StarListImage(detections=detections, image_metadata=fits_img.metadata)
    return solve_field(sources)


def test_astrometry_install() -> bool:
    """Test if the configured solve backend is properly installed."""
    config = get_or_initialize_config()
    if config.astrometry.solver_mode == "senpai":
        from senpai.astrometry.runner import examine_astrometry_install

        return examine_astrometry_install()
    return astroeasy.test_install(docker_image=config.astrometry.docker_image)


def require_astrometry_install() -> None:
    """Raise if the configured solve backend is not installed."""
    if not test_astrometry_install():
        raise ValueError("Astrometry solve backend is not installed or not in PATH")
...
def examine_indices() -> bool:
    ...
    config = get_or_initialize_config()
    if config.astrometry.solver_mode == "senpai":
        from senpai.astrometry import indices as senpai_indices

        return senpai_indices.examine_indices()
...
def enforce_indices() -> None:
    ...
    if config.astrometry.solver_mode == "senpai":
        ...
        senpai_indices.enforce_indices()
        return
