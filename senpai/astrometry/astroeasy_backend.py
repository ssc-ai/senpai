...
def solve_field_fits(fits_img: ProcessedFitsImage) -> StarField:
    """Plate-solve a preprocessed FITS image (the EOS detection-pipeline entry).
    ...
    if config.astrometry.solver_mode == "senpai":
        # In-process EOS solver: blind solve + same-cell verify + SIP-1 refine,
        # replicating `solve-field --verify --tweak-order 1` (see runner module).
        from senpai.astrometry.runner import solve_field as solve_field_eos

        return solve_field_eos(fits_img)
...
def examine_indices() -> bool:
    ...
    config = get_or_initialize_config()
    if config.astrometry.solver_mode == "senpai":
        from senpai.astrometry import indices as eos_indices

        return eos_indices.examine_indices()
