...
from senpai.engine.detection.streak.bayesian.masking import (
    analyze_source_shape_fwhm,
    map_cluster,
    map_cluster_v26,
    mask_all_but_border,
    remove_streak_at_point_enriched,
    remove_streak_at_point_v26,
)
...
def estimate_fwhm_from_profiles(x_profile: np.ndarray, y_profile: np.ndarray) -> tuple[float, float, float]:
    ...
    Args:
        x_profile: Summed profile along the x axis.
        y_profile: Summed profile along the y axis.
...
def is_valid_psf(
    ...
    Args:
        cutout: The extracted PSF cutout. Accepted for call-site compatibility;
            the overlap test reads the mask region, not the cutout itself.
        processed_mask: The mask of processed regions
        y_max: Row coordinate of the detected streak
        x_max: Column coordinate of the detected streak
...
def extract_streak_from_metadata(
    ...
    """Predict a frame's streak from its mount track rates rather than its pixels.

    Args:
        metadata: Frame metadata carrying the mount RA/Dec track rates.
        plate_scale_arcsec: Plate scale in arcseconds per pixel.
        wcs_model: WCS used to orient the rate vector in pixel space.
...
def prepare_rate_frame_v26(rate_frame: RateTrackFrame, padding: float = 0.05) -> np.ndarray:
...
def extract_streak_dims_mapping(
    ...
    """Extract a consensus streak measurement by mapping and clustering candidates.

    Repeatedly flood-fills the brightest sources to collect per-streak geometry,
    filters out round (non-streak) objects by aspect ratio, then finds the most
    self-consistent cluster of length/orientation to produce a single representative
    measurement. Round outliers are also removed from a second copy of the data.

    Args:
        data: Input image data to search for streaks.
        n_streaks: Target number of consistent streaks to base the measurement on.
    ...
    _length_mean, length_std = np.mean(lengths), np.std(lengths) or 1.0
    _orient_mean, orient_std = (
...
def extract_streak_dims_simple_long(
    ...
    streak_mask = map_cluster_v26(working_data, (y_max, x_max), threshold)
...
def _estimate_streak_seed(data: np.ndarray, fwhm: float = 4.0, crop: int = 2048) -> tuple[float, float]:
    ...
    def _no_response() -> None:
        """Signal that the matched-filter search found no usable response."""
...
def extract_streak_dims_robust_v26(
    data: np.ndarray,
    n_streaks: int = 10,
    length: float | None = None,
    rotation: float | None = None,
    ...
    # Sanity check on angle (disabled below)
...
def extract_streak_dims(
    ...
    """Extract streak rotation and length via matched filtering and PSF stacking.

    Clears bright border artifacts, matched-filters the data with a streak kernel, and
    stacks the highest-SNR streak cutouts into an idealized PSF from which the length is
    measured and refined along the streak axis. Falls back to the seed estimates when
    the refined length is implausibly short or no candidate is found.

    Args:
        data: Input image data to search for streaks.
        n_streaks: Maximum number of streak cutouts to stack into the PSF.
        length: Initial streak length estimate in pixels (used to size the kernel).
        rotation: Initial streak rotation estimate in degrees (used to size the kernel).
        fwhm: Estimated PSF FWHM in pixels used to size the matched-filter kernel width.
    ...
    while np.max(border_pixels) > pixel_cut:
        ...
        data = remove_streak_at_point_v26(data, start_point, fill_min)
        border_pixels = mask_all_but_border(data, 2)
    ...
    while len(streaks) < n_streaks and attempts < 30:
        ...
        if (
            ...
            conv = remove_streak_at_point_v26(conv, [maxx, maxy], cfill_min)
            data = remove_streak_at_point_v26(data, [maxx, maxy], fill_min)
            continue
    ...
    streak_len = streak_length_from_cutout_v26(psf)

    # in case weird case with extremely precise point sources
    streak_len = np.max([fwhm, streak_len])

    streak_len_refined = refine_streak_len_v26(psf, fwhm, rotation)
...
def streak_fwhm_from_cutout_v26(cutout_frame: np.ndarray, rotation: float) -> float:
    """Measure the Full Width at Half Maximum (FWHM) of a streak PSF.
...
def streak_length_from_cutout_v26(cutout_frame: np.ndarray, plot: bool = True) -> float:
    ...
    fill_min = 0.50  # FWHM
    start_point = np.unravel_index(np.argmax(subcc), subcc.shape)
    mapped = map_cluster_v26(subcc, start_point, fill_min)
...
def streak_parameters_from_xcorr_v26(
    cutout_frame: np.ndarray,
    plate_scale_arcsec: float,
    seeing_fwhm_pixels: float,
    ...
    for thresh in thresholds:
        try:
            start_point = np.unravel_index(np.argmax(subcc), subcc.shape)
            mapped = map_cluster_v26(subcc, start_point, thresh)
...
def estimate_fwhm_from_profiles_v26(x_profile: np.ndarray, y_profile: np.ndarray) -> float:
    """Estimate FWHM from profiles when curve fitting fails.
...
def cross_corr_v26(img1: np.ndarray, img2: np.ndarray) -> np.ndarray:
    """Cross correlate two images using FFT.
...
def measure_psf_fwhm_v26(data: np.ndarray, rotation: float | None = None) -> float:
    """Measure the FWHM of the PSF perpendicular to the streak direction with sub-pixel precision.
...
def _cached_working_bg(working_data: np.ndarray) -> float:
    ...
    The cache is weakref-validated against id reuse. Masking a handful of small
...
def mask_streak_region_v26(
    ...
    """Mask a streak region's area of influence in the mask and working data.

    Also optionally masks a ``response`` map used for peak-finding. For speed in the
    candidate loop, pass a precomputed ``effective_kernel``
    ...
    Args:
        processed_mask: The existing boolean mask of processed regions, updated in place.
        working_data: The image data to mask, updated in place.
        y_max: Row coordinate of the detected streak.
        x_max: Column coordinate of the detected streak.
        kernel: The detection kernel; self-convolved to form the influence area when
            ``effective_kernel`` is not supplied.
        threshold: Fraction of the effective kernel's max used to binarize it.
