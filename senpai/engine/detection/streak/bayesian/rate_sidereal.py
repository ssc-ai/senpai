...
from senpai.engine.detection.streak.bayesian.extraction import (
    cross_corr,
...
from senpai.engine.detection.streak.bayesian.rate_rate import bayesian_optimize_proposed_shift
...
def solve_rate_from_sidereal(
    ...
    # The registration kernel is sized from the sidereal frame's measured PSF scale. Without a
    # solved starfield there is no scale to size it from, so fail with the same typed error the
    # degenerate-timing case above uses rather than an AttributeError on None.
    starfield = sidereal_frame.starfield
    if starfield is None or starfield.detection_metadata is None:
        missing = "solved starfield" if starfield is None else "detection metadata"
        raise WcsPropagationError(
            f"Cannot register sidereal->rate anchor shift {frame_shift.source_index}->"
            f"{frame_shift.target_index}: the sidereal frame has no {missing}, so there is no "
            "measured PSF scale to size the registration kernel; no WCS can be propagated for "
            "this collect."
        )

    pixel_fwhm = starfield.detection_metadata.pixel_fwhm
