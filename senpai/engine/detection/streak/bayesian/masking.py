...
def percent_difference(a: float, b: float) -> float:
    """Percent difference between two values, relative to their mean.
...
def mask_tol(img: np.ndarray, center: tuple[int, int], pixel_tol: int = 30) -> np.ndarray:
    ...
    Args:
        img: Image whose shape the mask matches.
        center: ``(row, column)`` center of the circle.
        pixel_tol: Radius in pixels.
...
def map_cluster(
    ...
    Args:
        image: Image to flood-fill.
        start_point: ``(row, column)`` seed pixel.
        flux_threshold: Fill stops at pixels at or below this value.
        pad_size: Dilate the resulting mask by this many pixels.
...
def remove_brightest_streak(image: np.ndarray, fill_min: float) -> np.ndarray:
    ...
    Args:
        ...
        fill_min: Flux threshold bounding the fill.
...
def mask_all_but_border(image: np.ndarray, n_pixels: int = 1) -> np.ndarray:
    """Zero everything except an ``n_pixels`` frame around the image edge.

    Args:
        image: Source image (not modified).
        n_pixels: Width of the border to keep.
...
def mask_border(image: np.ndarray, n_pixels: int = 1) -> np.ndarray:
    """Zero an ``n_pixels`` frame around the image edge, keeping the interior.

    Args:
        image: Source image (not modified).
        n_pixels: Width of the border to zero.
...
def remove_n_brightest_streaks(image: np.ndarray, n: int) -> tuple[np.ndarray, int]:
    """Remove the ``n`` brightest streaks in turn.

    Args:
        image: Image to modify.
        n: Number of streaks to remove.
...
def map_cluster_with_peaks(
    ...
    Args:
        ...
        start_point: ``(row, column)`` seed pixel.
        flux_threshold: Fill stops at pixels at or below this value.
        pad_size: Dilate the cluster mask by this many pixels.
        min_separation: Minimum pixel separation between reported peaks.
...
def remove_streak_at_point_enriched(
    ...
    """Remove a streak by bounded flood fill, reporting what was filled.

    Args:
        image: Image to modify.
        start_point: ``(row, column)`` seed pixel of the streak.
        fill_min: Flux threshold bounding the fill.
        fill_mode: Reduction giving the replacement value.
        max_radius: Maximum fill radius in pixels; defaults to a bounded value.
        max_pixels: Maximum number of pixels to fill; defaults to a bounded value.
...
def remove_border_crossing_streaks_pairwise(
    ...
    Args:
        ...
        drift_dx: Expected inter-frame drift in x (sign-agnostic).
        drift_dy: Expected inter-frame drift in y (sign-agnostic).
...
def remove_streak_at_point_v26(
    image: np.ndarray,
    start_point: tuple[int, int],
    fill_min: float,
    fill_mode: np.ufunc = np.mean,
...
def remove_border_crossing_streaks_v26(image: np.ndarray) -> np.ndarray:
    """Remove streaks that cross or touch the image border.
