...
from senpai.engine.detection.streak.bayesian.rate_rate import solve_rate_from_rate
from senpai.engine.detection.streak.bayesian.rate_sidereal import solve_rate_from_sidereal
from senpai.engine.detection.streak.bayesian.sidereal_sidereal import solve_sidereal_from_sidereal
from senpai.engine.detection.streak.bayesian.validation import (
    validate_proposed_shift_v26 as validate_proposed_shift,
)
...
def preprocess_for_shift(frame: ProcessedFitsImage) -> None:
    ...
    if ProcessingStep.ROW_MEDIAN_SUBTRACT not in applied or ProcessingStep.COLUMN_MEDIAN_SUBTRACT not in applied:
        ...
        # ADU scale (see calibrations.preprocess_float_dtype).
        frame = remove_column_and_row_medians(frame, dtype=preprocess_float_dtype())
