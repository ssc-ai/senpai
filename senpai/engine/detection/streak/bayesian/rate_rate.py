...
from senpai.engine.detection.streak.bayesian.validation import validate_proposed_shift
...
def windowed_correlation_peaks(
    ...
    cluster after masking a single pixel -- can lock onto the wrong peak (a failure mode seen on our benchmark). The mask/window are clamped positive so the search region is never empty (a
...
def solve_rate_from_rate(
    ...
    # Symmetric pairwise border removal when drift is known (upstream feature, config-gated); else per-frame.
    drift_axis = None
    ...
    if (
        ...
        rate_a_data, rate_b_data, filled_a, filled_b = remove_border_crossing_streaks_pairwise(
            rate_a_data, rate_b_data, drift_mag * drift_axis[0], drift_mag * drift_axis[1], pad_px
        )
