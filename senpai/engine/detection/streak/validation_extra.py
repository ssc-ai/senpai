"""Preserved v2.6 lightweight shift-validation helpers for streak detection.

Provides fast box-statistics correlation between frame pairs (in place of flood-fill)
and a random-shift comparison used to validate a proposed inter-frame shift.
"""
...
def extract_box_statistics(
    ...
    Returns:
        dict with 'max', 'sum', 'mean', 'median', and 'valid' keys. When the box
        falls outside the image, all statistics are ``0.0`` and 'valid' is False.
    """
...
def quick_correlation_from_boxes(
    ...
    Args:
        ...
        shift_x: X shift (pixels) to apply to source positions.
        shift_y: Y shift (pixels) to apply to source positions.
