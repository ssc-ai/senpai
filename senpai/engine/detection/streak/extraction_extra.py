"""Preserved v2.6 streak extraction and PSF-measurement helpers.

Bundles the frame-preparation, matched-filter streak-geometry, PSF-refinement, and
cross-correlation shift utilities used by the rate/sidereal streak shift solvers. This
is the extended ("extra") variant of :mod:`senpai.engine.detection.streak.extraction`
and carries additional robust extractors and cross-correlation centroiders.
"""
...
def extract_streak_from_metadata(
    ...
    """Predict a streak measurement from header track rates and the WCS.

    Computes the expected streak length from the RA/Dec track rates and exposure time,
    then projects the track-rate direction through the WCS PC matrix to derive the
    streak rotation in the image plane (normalized to ``[0, 180)``).

    Args:
        metadata: Frame metadata carrying the RA/Dec track rates and exposure time.
        plate_scale_arcsec: Plate scale in arcsec/pixel, used to convert the streak
            length from arcseconds to pixels.
        wcs_model: WCS model supplying the PC matrix that maps RA/Dec to pixel space.

    Returns:
        A ``StreakMeasurement`` with the predicted rotation and length (FWHM left
        ``None``), or ``None`` when the track rates or exposure time are missing.
    """
    if (
...
def refine_robust_streak(
    ...
    from senpai.engine.detection.streak.masking_extra import map_cluster
...
def _cached_working_bg(working_data: np.ndarray) -> float:
    """Return the median background of ``working_data``, cached by array identity.

    Cached by array identity (weakref-validated against id reuse). Masking a handful of
    small regions doesn't shift a 66 MP median, so computing it once per extraction (not
    ~30x) is exact enough and avoids ~0.8 s/call.
    """
    import weakref
