"""Preserved v2.6 satellite point-source detection for rate-track frames (WCS fit)."""
...
from senpai.engine.detection.streak.masking_extra import percent_difference
...
def _dao_sources_at_threshold(
    ...
    *,
    ...
    """Return the exact ``DAOStarFinder(...)(data_sub)`` result at ``threshold``.

    Reuses a shared convolution and precomputed local maxima. Returns None when no
    candidate survives the threshold or DAO's property filters.
...
def cutout_gauss(
    ...
    def _validate_fit() -> None:
        """Raise ``ValueError`` if the fit did not converge to realistic sigma values."""
        # Fit may not have converged; check if the resulting sigmas are reasonable
        if fit_p.fit_info["ierr"] not in [1, 2, 3, 4] and (
...
def generate_cutout(
    ...
    Args:
        ...
        plot: Whether to save a PNG preview of the cutout. Defaults to False.
...
def filter_point_sources(
    ...
    for idx, detection in enumerate(detections):
        ...
        try:
            ...
            if config.detection.verbose:
                logger.warning(
                    f"[{idx + 1}] [FILTERING] Failed to fit Gaussian: {e!s}"
                )
            continue
