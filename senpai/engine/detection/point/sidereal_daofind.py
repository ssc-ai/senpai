...
def _detect_sources_daofind(
    ...
    try:
        ...
        logger.warning("Memory error while calling daofind. Proceeding as if no sources were detected.")
        sources = None
...
def extract_point_sources_daofind(
    ...
    for source in detected_sources:
        ...
        fwhm = estimate_fwhm(image.data, x_centroid, y_centroid, fwhm_x_guess=fwhm_guess, fwhm_y_guess=fwhm_guess)
        if fwhm is not None:
    ...
    sources = _detect_sources_daofind(image.data, max_sources=max_detections * 2, fwhm=fwhm_pixel, threshold_sigma=5.0)
