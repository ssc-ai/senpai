...
def _detect_sources_sextractor(
    ...
    a background-limited sidereal frame.
...
def estimate_fwhm_classic(
    ...
    try:
        ...
        logger.warning(f"Error estimating FWHM: {e}")
        return None
...
def sat_level_from_peaks(peaks: np.ndarray | list[float]) -> float:
    """Saturation level from a sample of source core peaks.

    See ``_estimate_saturation_level`` for the rationale. Shared with the
...
def _robust_source_fwhm(
    ...
    """Fit-free, saturation-aware FWHM.

    The diameter of the connected above-half-max region around the centroid.
...
def extract_point_sources_classic(
