"""SEP + flux-concentration rate-track point detector (opt-in).

Alternative rate-frame point-source detector selected by
``detection.rate_point_detector = "sep"`` (the dispatch lives in
``satellite.extract_point_sources``; the default ``"daostarfinder"`` keeps the
upstream DAOStarFinder path). Uses SEP for background/extraction and a
multi-stage PSF filter (flux concentration, roundness, seeing-relative FWHM) to
reject hot pixels, cosmic rays and extended blobs. MDP-validated on rate-track
imagery. Self-contained: its filter helpers are private to this module so the
DAOStarFinder detector in ``satellite.py`` is untouched.
"""
...
# Accept band for ``psf_flux_concentration``, derived from a rate-track validation corpus:
...
def measured_pixel_fwhm(frame: RateTrackFrame) -> float:
    ...
    if detection_metadata is not None and _valid(getattr(detection_metadata, "pixel_fwhm", None)):
...
def extract_point_sources_sep(frame: RateTrackFrame) -> SatelliteListImage:
