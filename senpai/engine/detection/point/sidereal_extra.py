"""Preserved v2.6 point-source detection and FWHM utilities for sidereal frames.

Provides DAOStarFinder-based source detection, robust saturation-aware FWHM estimation,
sub-pixel centroid refinement, and the two-pass ``extract_point_sources`` entry point.
"""
...
def gaussian_2d(
    ...
    """Evaluate a rotated 2D Gaussian for curve fitting.

    Args:
        data: Tuple of ``(x, y)`` coordinate grids at which to evaluate the model.
        amp: Peak amplitude above the offset.
        x0: Center x (column) position.
        y0: Center y (row) position.
        sigma_x: Standard deviation along the (unrotated) x axis.
        sigma_y: Standard deviation along the (unrotated) y axis.
        theta: Rotation angle of the Gaussian in radians.
        offset: Constant additive background level.

    Returns:
        The flattened Gaussian values, matching a raveled cutout for ``curve_fit``.
    """
    x, y = data
    x0 = float(x0)
    y0 = float(y0)
...
def sat_level_from_peaks(peaks: list[float]) -> float:
    """Return a saturation level from a sample of source core peaks.

    See :func:`_estimate_saturation_level` for the rationale. Shared with the
...
def _robust_source_fwhm(
    ...
    """Measure a fit-free, saturation-aware FWHM around a centroid.

    Returns the diameter of the connected above-half-max region around the centroid.
...
def extract_point_sources(
    ...
    """Detect point sources in a sidereal frame with an adaptive two-pass scheme.

    Pass 1 samples bright sources to estimate the median PSF FWHM and the frame's
    saturation level. Pass 2 detects all sources at the adapted FWHM (optionally on a
    2x2-binned frame for fat PSFs), refines binned centroids to sub-pixel accuracy at
    full resolution, and applies a minimum-separation cut.

    Args:
        image: The processed FITS image to detect sources in.
        max_detections: Maximum number of point sources to return. Defaults to 100.
        min_separation: Minimum separation between detections in pixels. When None, a
            value is derived from the measured FWHM.

    Returns:
        A tuple of the detected ``StarListImage`` and the measured pixel FWHM.
    """
