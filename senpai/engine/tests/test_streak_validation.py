...
def _star_field(n_stars: int = 30, seed: int = 0, margin: int = 60):
...
def _render(positions, dx: float = 0.0, dy: float = 0.0, noise: float = 3.0, seed: int = 7):
...
def _catalog(positions) -> list[StarInSpace]:
...
class TestExtractBoxStatistics:
    def test_returns_valid_stats_for_interior_point(self):
...
class TestQuickCorrelation:
    def test_true_shift_correlates_strongly(self):
    ...
    def test_wrong_shift_correlates_poorly(self):
        positions = _star_field(n_stars=30, seed=1)
        dx, dy = 20, -15
        source = _render(positions, dx=0, dy=0, seed=2)
        target = _render(positions, dx=dx, dy=dy, seed=3)
    ...
    def test_too_few_valid_stars_returns_zero(self):
    ...
    def test_respects_max_stars_limit(self):
...
class TestValidateShift:
    ...
    def test_true_shift_validates(self):
        dx, dy = 20, -15
    ...
    def test_insufficient_stars_rejected(self):
    ...
    def test_validate_proposed_shift_delegates(self):
        dx, dy = 18, 12
        source, target, cat = self._pair(dx, dy)
    ...
    def test_zero_shift_with_aligned_frames_validates(self):
    ...
    def test_fwhm_exclusion_path_validates_true_shift(self):
