...
def _write_fits(path: Path, header_items: dict | None = None) -> Path:
    """Write a FITS file with chosen header keywords, for the organization helpers to read."""
...
def test_uct_time_from_combined_date_obs() -> None:
    """A combined date-time keyword parses."""
...
def test_uct_time_from_separate_date_and_time() -> None:
    ...
    """A separate date and time pair parses."""
...
def test_uct_time_date_only_without_time_raises() -> None:
    ...
    """A date with no time raises rather than assuming midnight.

    Assuming a time would place every frame of the night at the same instant, and collect
    grouping is built on those timestamps.
    """
...
def test_uct_time_no_date_headers_raises() -> None:
    """A header with no date at all raises."""
    with pytest.raises(AttributeError):
...
def test_uct_time_prefers_date_time_over_separate() -> None:
    ...
    """A combined keyword takes precedence over the separate pair when both are present."""
    header = {
...
def test_get_imageset_by_filename_matches_substring(tmp_path: Path) -> None:
    """Files are matched on a filename substring."""
...
def test_get_imageset_by_filename_recurses_subdirs(tmp_path: Path) -> None:
    """The search recurses into subdirectories."""
    sub = tmp_path / "night1"
    sub.mkdir()
...
def test_get_imageset_by_filename_no_match(tmp_path: Path) -> None:
    """No match yields an empty list rather than every file."""
...
def test_get_all_images_in_directory_sorted(tmp_path: Path) -> None:
    """Every FITS file is returned, sorted.

    Order is the order frames were taken, and registration walks them in sequence.
    """
...
def test_extract_id_from_header_plain_key(tmp_path: Path) -> None:
    """A plain keyword yields its value as the collect id."""
...
def test_extract_id_from_header_missing_key_returns_none(tmp_path: Path) -> None:
    """A missing keyword yields None rather than raising."""
...
def test_extract_id_from_header_orchcomm_parsing(tmp_path: Path) -> None:
    """The packed command keyword has its id parsed out of the longer string."""
...
def test_header_key_matches_true_and_false(tmp_path: Path) -> None:
    """Header matching answers true and false as the value dictates."""
...
def test_header_key_matches_missing_key_false(tmp_path: Path) -> None:
    """A missing keyword does not match, rather than raising."""
...
def test_header_key_matches_orchcomm_substring(tmp_path: Path) -> None:
    """The packed command keyword matches on the id inside it, not the whole value."""
...
def test_get_imageset_by_id_groups_matching_headers(tmp_path: Path) -> None:
    """Frames sharing a header id are grouped into one collect."""
...
def test_get_imageset_by_id_no_match_empty(tmp_path: Path) -> None:
    """An id no frame carries yields an empty list."""
    _write_fits(tmp_path / "f1.fits", {"IMAGESET": "night-A"})
