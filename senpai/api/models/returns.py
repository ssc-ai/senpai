...
class AstrometrySummary(BaseModel):
    ...
    def serialize_floats(self, v: float | None) -> float | None:
        """Round to 6 decimals -- microdegrees, well below any solve's uncertainty."""
...
class PhotometryResult(BaseModel):
    ...
    def serialize_floats(self, v: float | None) -> float | None:
        """Round to millimagnitudes, finer than any real photometric precision."""
...
class SeeingResult(BaseModel):
    ...
    def serialize_floats(self, v: float | None) -> float | None:
        """Round to hundredths of a pixel or arcsecond."""
...
class FrameDetection(BaseModel):
    ...
    def serialize_px(self, v: float) -> float:
        """Round pixel coordinates to hundredths -- finer than any centroid is good to."""
        return round(v, 2)

    @field_serializer("ra_deg", "dec_deg")
    def serialize_radec(self, v: float | None) -> float | None:
        """Round to 6 decimals, about 4 milliarcseconds at the equator."""
        return round(v, 6) if v is not None else None

    @field_serializer("snr", "fwhm_px", "streak_length_arcsec", "streak_angle_deg", "streak_rate_arcsec_per_sec")
    def serialize_floats(self, v: float | None) -> float | None:
        """Round the measured streak and detection quantities to hundredths."""
        if v is None:
            return None
        return round(v, 2)
...
def _seeing_from_frame(
    frame: "SiderealFrame | RateTrackFrame", pixel_scale_arcsec: float | None = None
) -> SeeingResult:
