...
def main(argv: list[str] | None = None) -> int:
    ...
    totals = replot([Path(p) for p in args.paths], kinds=kinds, force=args.force, gifs=not args.no_gifs)
    if not totals:
        print(f"No batch directories found under: {', '.join(str(p) for p in args.paths)}")
        return 1
